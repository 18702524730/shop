<template>
  <div class="dlqyForm">
    <el-form ref="ruleForm" :rules="rules" :model="conForm" label-width="172px">
      <!--大陆企业-->
      <div>
        <el-form-item label="申请企业名称：" prop="apply_name">
          <el-input v-model ="conForm.apply_name"></el-input>
        </el-form-item>
        <el-form-item label="企业营业执照编号：" prop="apply_number">
...
export default {
  ...
  data () {
    ...
    let dlqy_checkNumber = (rule, value, callback) =>{
      ...
      if (!value) {
          return callback(new Error('企业营业执照编号不能为空'));
      }
      setTimeout(() => {
          if (!reg.test(value)) {
            callback(new Error('企业营业执照编号格式错误'));
          } else {
            callback();
          }
      }, 300);
    ...
    return {
      ...
      rules:{
        //大陆企业
        apply_name:[
          { required: true, message: '请输入申请企业名称', trigger: 'blur' },
