...
.search_container{
	...
	.breadcrumb_container{position: relative;
		.result_num{position: absolute;right: 0; top: 0;
			span{color:#00B3FF;}
			i { 
				width: 18px; height: 18px; line-height: 18px; background-color: #ccc; color: #fff; text-align: center; border-radius: 50%; font-size: 12px; margin-left: 10px; cursor: pointer;
				&:hover {background-color: #3DB1FA;}
			}
		}
	}
	.search_type{padding: 0 20px 0 20px;font-size: 12px;color:#333;border:1px solid rgba(232,232,232,1);margin-top: 20px; position: relative;
		.border_fold {position: absolute; height: 1px; border: 1px solid #e8e8e8; left: 1px; right: 1px; bottom: -3px; box-sizing: content-box; border-radius: 1px; }
		.one_level{border-bottom: 1px dashed #eee;
			...
			>div{
				width: 1038px;
				.level_item{
					padding: 5px 0 5px 0; border: 1px solid #fff;
	...
	.search_main{
		...
		.search_result{
			.result_item{
				width: 210px;font-size: 0;margin: 0 27.5px 20px 0;border:1px solid rgba(237,237,237,1); cursor: pointer;
				&:nth-child(5n+5){margin-right: 0;}
				&.trademarkItem {
					.full {
						wdith:210px; height:210px; overflow:hidden; display:inline-block;
						.img{
							width: 210px;height: 210px; transition: all 0.3s; padding: 0;
				...
				.item_des{
					...
					>a{display:block;text-decoration: none;
						&.item_name{line-height: 20px;padding: 5px 0;color: #333;font-size: 14px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;
							span{color: #333;}
						}
						&.shop_name{line-height: 17px;color: #BBBBBB;font-size: 12px;padding-bottom: 8px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis;}
					}
					.item_icon{
						height: 25px;
		...
		<div class="search_container">
			<div class="breadcrumb_container">
				<el-breadcrumb separator-class="el-icon-arrow-right">
					...
					<el-breadcrumb-item v-if="selectFirst">{{ selectFirst }}</el-breadcrumb-item>
					<el-breadcrumb-item v-if="selectSecond">{{ selectSecond }}</el-breadcrumb-item>
					<el-breadcrumb-item v-if="selectThird">{{ selectThird }}</el-breadcrumb-item>
				</el-breadcrumb>
			...
			<div class="search_type" >
				<div class="one_level clearfix" :class="{'border_bottom': !showAllType || !listSecond.length}">
					<p class="fl">一级分类:</p>
					<div class="fl">
				...
				<div class="one_level clearfix" v-if="listSecond && listSecond.length">
					<p class="fl">二级分类:</p>
					<div class="fl">
						<ul class="fl clearfix level_item">
				...
				<div class="one_level clearfix noBorder" v-if="listThird && listThird.length">
					<p class="fl">三级分类:</p>
					<div class="fl">
			...
			<div class="search_main">
				...
				<div class="search_recommend" v-if="!totalSize">
				...
				<div class="search_result clearfix" v-if="searchList.length">
					<div class="result_item trademarkItem fl" v-for="item in searchList">
						<a :href="indexUrl + '/store/service/'+ item.code + '.htm'" target="_blank" class="full">
							<img :src="item.picUrl" :onerror="defaultImg" class="img" />
						</a> 
						<div class="item_des">
							<p class="item_price">
								<span v-if="item.priceType!=3">￥{{ item.amount | priceformat}}</span>
								<span v-if="item.priceType==3">价格面议</span>
							...
							<a class="item_name" :href="indexUrl + '/store/service/'+ item.code + '.htm'" target="_blank" :style="{color: keyword == item.serviceTitle ? '#3DB1FA' : '#333'}">{{ item.serviceTitle }}</a>
...
export default{
	...
	methods: {
		// 去服务详情页
		goDetailFn (item) {
			location.href = indexUrl + '/store/service/'+ item.code + '.htm';
		},
		// 上一页
		prevePageFn () {
			...
			this.searchFn({pg_index: this.currentPage});
		},
		// 下一页
		nextPageFn () {
		...
		selectTypeFn (type) {
			...
			this.sort = 'desc'
			this.priceName = '价格';
			this.searchFn({sort_type: this.sortType, sort: this.sort});
		},
		// 选择价格培训类型
		commandFn (command) {
			this.sortType = 3;  //价格
		...
		getServiceTypeListFn () {
			// this.allBsList = JSON.parse(localStorage.getItem('findAllBsList'));
		...
		// 搜索服务
		searchFn (obj) {
			let query_ = Object.assign({}, this.$route.query, obj)
		...
		searchQueryFn () {
			...
			if(this.$route.query.pg_index) {
				this.currentPage = parseInt(this.$route.query.pg_index);
			}
			// 一级分类
			if (this.$route.query.firstCategory) {
				this.firstCategory = this.$route.query.firstCategory;
				let index_ = this.getIndex(this.allBsList, this.firstCategory);
				if (this.allBsList[index_]) {
					this.listSecond = this.allBsList[index_].childs;
					this.selectFirst = this.allBsList[index_].name;  //一级分类名称
				}
			}
			else{
				this.firstCategory = '';
			...
			// 二级分类
			if (this.$route.query.secondCategory) {
				this.secondCategory = this.$route.query.secondCategory;
				let index_ = this.getIndex(this.listSecond, this.secondCategory);
				if (this.listSecond[index_] ) {
					this.listThird = this.listSecond[index_].bsList;
					this.selectSecond = this.listSecond[index_].name;  //二级分类名称
				}
			}
			else{
				this.secondCategory = '';
			...
			// 三级分类
			if (this.$route.query.thirdCategory) {
				this.thirdCategory = this.$route.query.thirdCategory;
				for (var i = 0; i < this.listThird.length; i++) {
					if (this.listThird[i].bs_id == this.thirdCategory) {
						this.selectThird = this.listThird[i].name;  //三级分类名称
						break;
					}
				}
			}
			...
			let params = Object.assign({}, query);  // 这样算是生成了一个新对象，不过这个方法只能深拷贝一维对象，如果不深拷贝一下的话操作这个params就相当于操作了this.$route路由对象中的query对象，那样就间接改变了路由。
			params.pg_count = query.pageSize || this.pageSize;
			params.keyword = query.keyword ? decodeURIComponent(decodeURIComponent(query.keyword)) : '';
			params.firstCategory = parseInt(query.firstCategory);
			params.secondCategory = parseInt(query.secondCategory);
			params.thirdCategory = parseInt(query.thirdCategory);
			this.$http.post(searchServiceUrl, params)
			.then( (resp) =>{
	...
	mounted () {
		this.getServiceTypeListFn(); //查询所有分类数据
	},
	filters: {
