			...
			.result_item{
				...
				.item_des{
					...
					>a{display:block;text-decoration: none;
						&.item_name{line-height: 20px;padding: 5px 0;color: #3DB1FA;font-size: 14px; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; cursor: pointer;
							span{color: #333;}
						}
		...
		<div class="search_container">
			...
			<div class="search_type" v-show="showAllType">
				...
				<div class="one_level clearfix noBorder ">
					<p class="fl">商标分类:</p>
					<div class="fl">
						<ul class="fl clearfix level_item" :class="{'line': !showTmType}">
			...
			<div class="search_main">
				<div class="search_nav">
					<!-- <div class="fl clearfix sorting mr20">
						<p class="fl bor_r" :class="{'select':sort<2}" @click="composite">综合排序</p>
						<p class="fl bor_r sort_img" :class="{'select': 2<=sort && sort<=3}" @click="priceSort">价格
							<img v-if="sort<2 || sort>3" src="~assets/img/icon/sort.png" alt="">
							<img v-if="2<=sort && sort<=3" :class="{'to_add':sort == 2,'to_move':sort == 3}" src="~assets/img/icon/rank.png" alt="">
						</p>
						<p class="fl bor_r sort_img" :class="{'select': 3<sort && sort<=5}" @click="timeSort">下证时间
							<img v-if="sort<=3 || sort>5" src="~assets/img/icon/sort.png" alt="">
							<img v-if="3<sort && sort<=5" :class="{'to_add':sort == 4,'to_move':sort == 5}" src="~assets/img/icon/rank.png" alt="">
						</p>
						<p class="fl sort_img" :class="{'select':5<sort && sort<=7}" @click="wordSort">字符数量
							<img v-if="sort<=5 || sort>7" src="~assets/img/icon/sort.png" alt="">
							<img v-if="5<sort && sort<=7" :class="{'to_add':sort == 6,'to_move':sort == 7}" src="~assets/img/icon/rank.png" alt="">
						</p>
					</div> -->
				...
				<div class="search_result clearfix">
					<div class="result_item fl" v-for="item in searchData">
						<img :src="item.picture" />
						<div class="item_des">
							<p class="item_price"><span>￥{{ item.seller_price }}</span><a href="javascript:void(0)" class="fr"><img src="~assets/img/icon/chat.png" /></a></p>
							<a class="item_name">{{ item.trademark_name }}</a>
				...
				<div class="search_recommend" v-if="!searchData.length">
					<h2>为您推荐</h2>
					<p>由于符合<span>"国内商标注册申请"</span>的结果较少，尝试为您推荐以下搜索的结果</p>
				</div>
				<div class="search_result clearfix" v-if="!searchData.length">
...
export default{
	...
	methods: {
		...
		routeChange(){//搜索参数，动态改变路由
			...
			this.$http.post(searchTmUrl, data)
			.then( (resp) => {
		...
		trademarkLists(){//查询列表
			...
			this.$http.post(premarketUrl + '/trademark/search',this.$route.query)
			.then((resp)=>{
				this.searchData = resp.data.data;
				this.total = resp.data.total_count;
			})
		...
		searchRoute(){
			...
			if(this.$route.query){//因为参数不在一个data中，且需要对参数区间进行处理，只能一个个对应
				...
				console.log(data.minWordCount,8888888888888)
